...
export default function ContactUs({ currentUser, onNavigate }: ContactUsProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!name.trim() || !email.trim() || !subject.trim() || !message.trim()) {
      setError('Please fill in all required fields so we can receive your message.');
      return;
    }
    ...
    setMessage('');
    setError('');
    setSuccess('Your message was sent successfully! Our support team will get back to you as soon as possible. 🌫️');
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 text-left min-h-[80vh] select-none">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-8 border-b border-white/5 pb-6">
        ...
        <div className="flex items-center gap-3">
          <div>
            ...
            <p className="text-[10px] text-purple-400 mt-1">Reach the MistVil team directly and we’ll reply as fast as we can</p>
          </div>
        </div>
      </div>
      ...
      {error && (
        <div className="p-4 bg-red-500/15 border border-red-500/30 text-red-400 text-xs font-bold rounded-2xl mb-6 flex items-center justify-start gap-2">
          <span>{error}</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
          ...
          <div className="glass-panel p-6 rounded-2xl border border-white/5">
            <h3 className="text-sm font-bold text-white mb-4 flex items-center gap-2 justify-start">
              <span>Official support & contact channels</span>
              <MessageSquare size={16} className="text-violet-400" />
            </h3>
            <p className="text-xs text-purple-200/85 mb-4 leading-relaxed">
              If you hit a technical issue, or want to ask about a translation or partnership, use the form beside this or reach us directly through the channels below:
            </p>
            <div className="flex flex-col gap-3">
              ...
              <div className="flex items-center justify-between bg-white/5 p-3 rounded-xl border border-white/5">
                <span className="text-xs font-bold text-violet-400">Official Discord ticket</span>
                <span className="text-xs text-purple-300 font-bold">Live technical support</span>
              </div>
              <div className="flex items-center justify-between bg-white/5 p-3 rounded-xl border border-white/5">
                <span className="text-xs text-purple-100 font-bold">24 hours / 7 days a week</span>
                <span className="text-xs text-purple-300 font-bold">Working hours</span>
              </div>
            </div>
          </div>
            ...
            <div className="glass-panel p-6 rounded-2xl border border-violet-500/20 bg-violet-950/10">
              <h3 className="text-sm font-bold text-white mb-3 flex items-center justify-between">
                <span className="flex items-center gap-1.5">
                  ...
                  <span>Received admin mail ({messages.length})</span>
                </span>
                <span className="text-[9px] bg-violet-500/25 text-violet-300 px-1.5 py-0.5 rounded-full font-bold">Owner only</span>
              </h3>
              
              {messages.length === 0 ? (
                <div className="p-8 text-center bg-white/5 rounded-xl border border-white/5">
                  <p className="text-xs text-purple-400">No new messages yet.</p>
                </div>
              ) : (
                <div className="flex flex-col gap-3 max-h-[350px] overflow-y-auto pr-1">
                  {messages.map((msg) => (
                    <div key={msg.id} className="p-4 bg-white/5 hover:bg-white/10 rounded-xl border border-white/5 transition-all text-left relative group">
                      <button
                        onClick={() => handleDeleteMessage(msg.id)}
                        className="absolute right-2 top-2 p-1.5 text-red-400 hover:text-red-300 bg-red-500/10 hover:bg-red-500/20 rounded-lg transition-all"
                        title="Delete message"
                      >
                        <Trash2 size={12} />
                      </button>
                      <div className="flex items-center gap-2 mb-1 justify-start pl-8">
        ...
        <div className="lg:col-span-7">
          <form onSubmit={handleSubmit} className="glass-panel p-6 md:p-8 rounded-2xl border border-white/5 flex flex-col gap-4">
            <h2 className="text-base font-bold text-white mb-2">Quick Contact Form ✍️</h2>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5 text-left">
                <label className="text-[11px] text-purple-300 font-bold">Your name / username</label>
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-[#131F33] border border-white/10 focus:border-violet-500 outline-none rounded-xl py-2 px-3 text-white text-xs text-left"
              ...
              <div className="flex flex-col gap-1.5 text-left">
                <label className="text-[11px] text-purple-300 font-bold">Your email address</label>
                <div className="relative">
                  <input
                    type="email"
                    required
            ...
            <div className="flex flex-col gap-1.5 text-left">
              <label className="text-[11px] text-purple-300 font-bold">Subject</label>
              <input
                type="text"
                required
                value={subject}
                ...
                className="w-full bg-[#131F33] border border-white/10 focus:border-violet-500 outline-none rounded-xl py-2 px-3 text-white text-xs text-left"
                placeholder="e.g. Partnership request / report a chapter issue"
              />
            </div>

            <div className="flex flex-col gap-1.5 text-left">
              <label className="text-[11px] text-purple-300 font-bold">Message details</label>
              <textarea
                required
                rows={6}
                value={message}
                onChange={(e) => setMessage(e.target.value)}
                className="w-full bg-[#131F33] border border-white/10 focus:border-violet-500 outline-none rounded-xl p-3 text-white text-xs text-left leading-relaxed resize-y"
